...
[analisando antes de mim!](http://dvatvani.github.io/BGG-Analysis-Part-1.html) Pelo menos me serviu de inspiração pra começar. Assim, deixo avisado de antemão que algumas análises podem ser semelhantes mas quero dar foco diferente (e em português!).
...
quant1 <- filter(dados, expansion == 'N') %>% top_n(own, n=50)
 ...
 ggplot(quant1, aes(y = own, x = reorder(title, own))) +
  ...
  labs(title="Número de coleções que possuem o jogo", x ="Jogo", y = "Quantidade de coleções",
       caption = 'Dados extraídos de https://boardgamegeek.com')
...
Tudo bem, temos um monte de jogos, mas não tenho tempo de jogar todos eles, infelizmente. Pra saber qual vale a pena sempre vemos o *ranking* dos jogos, mas a dúvida que fica, os jogos mais novos são melhores que os mais antigos? Bem, talvez.
...
Essa é a sessão que eu decidi ser, a "visualização ambiciosa do dia". Fiz uma mistura de informação pra ver se temos algum *insight* interessante. Os jogos do BGG possuem [51 mecânicas possíveis](https://boardgamegeek.com/browse/boardgamemechanic), onde um jogo pode ser construído utilizando diversas mecânicas mas, um jogo com mais mecânicas influi diretamente num jogo mais pesado (complexo)?
